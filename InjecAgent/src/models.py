...
from src.llm_client import ATTACK_MODEL, LLM_TEMPERATURE, chat_completion_with_retry
...
class GPTModel(BaseModel):
    def __init__(self, params):
        super().__init__()
        self.params = params
        self.model_name = params.get('model_name') or ATTACK_MODEL
    ...
    def call_model(self, model_input):
        content, error, _ = chat_completion_with_retry(
            messages=model_input,
            model=self.model_name,
            temperature=LLM_TEMPERATURE,
            label=f"victim:{self.model_name}",
        )
        if error:
            return None
        return content
